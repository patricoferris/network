...
let to_bigarray buffer = Bigarray.Array1.sub buffer.buffer buffer.off buffer.len

let create_unsafe len =
  let buffer = Bigarray.(Array1.create char c_layout len) in
  { buffer; len; off = 0 }

let check_bounds t len = len >= 0 && Bigarray.Array1.dim t.buffer >= len
...
external ba_set_int16 : buffer -> int -> uint16 -> unit
  = "%caml_bigstring_set16u"

external ba_set_int32 : buffer -> int -> uint32 -> unit
  = "%caml_bigstring_set32u"

external ba_set_int64 : buffer -> int -> uint64 -> unit
...
let string ?(off = 0) ?len str =
  ...
  if off < 0 || len < 0 || off + len > str_len then
...
let buffer ?(off = 0) ?len buffer =
  ...
  if off < 0 || len < 0 || off + len > buffer_len then
    invalid_arg "index out of bounds";
  of_bigarray ~off ~len buffer
...
let start { buffer; off; _ } = of_bigarray buffer ~off ~len:0
let stop { buffer; off; len } = of_bigarray buffer ~off:(off + len) ~len:0
...
let trim ?(drop = is_white) ({ buffer; off; len } as cs) =
  if len = 0 then cs
    ...
    let rec left_pos i =
      ...
      else if drop (get_char cs i) then left_pos (succ i)
      else i
    in
    let rec right_pos i =
      if i < 0 then 0
      else if drop (get_char cs i) then right_pos (pred i)
      else succ i
    in
    let left = left_pos 0 in
    if left = max_zpos then
      of_bigarray buffer ~off:(((off * 2) + len) / 2) ~len:0
    else
...
let fspan ~min ~max ~sat ({ buffer = v; off; len } as cs) =
  ...
  if min > max || max = 0 then (buffer ~off ~len:0 v, cs)
  else
    let max_zidx = len - 1 in
    let max_zidx =
      let k = max - 1 in
      if k > max_zidx || k < 0 then max_zidx else k
    in
    let need_zidx = min in
    let rec loop i =
      if i <= max_zidx && sat (get_char cs i) then loop (i + 1)
      else if i < need_zidx || i = 0 then (buffer ~off ~len:0 v, cs)
      else if i = len then (cs, buffer ~off:(off + len) ~len:0 v)
      else (buffer ~off ~len:i v, buffer ~off:(off + i) ~len:(len - i) v)
    in
    loop 0
...
let rspan ~min ~max ~sat ({ buffer = v; off; len } as cs) =
  ...
  if min > max || max = 0 then (cs, buffer ~off:(off + len) ~len:0 v)
    ...
    let rec loop i =
      ...
      else if i > need_zidx || i = max_zidx then
        (cs, buffer ~off:(off + len) ~len:0 v)
      else if i < 0 then (buffer ~off ~len:0 v, cs)
      else
        ( buffer ~off ~len:(i + 1) v,
          buffer ~off:(off + i + 1) ~len:(len - (i + 1)) v )
    in
    loop max_zidx
...
let fcut ~sep:({ len = sep_len; _ } as sep) ({ buffer = v; off; len } as cs) =
  ...
  let rec check_sep i k =
    ...
    else if get_char cs (i + k) = get_char sep k then check_sep i (k + 1)
    else scan (i + 1)
  and scan i =
    if i > max_s_zidx then None
    else if get_char cs i = get_char sep 0 then check_sep i 1
    else scan (i + 1)
  in
  scan 0
...
let cut ?(rev = false) ~sep cs =
  match rev with true -> rcut ~sep cs | false -> fcut ~sep cs

let add_sub ~no_empty buf ~off ~len acc =
  if len = 0 then if no_empty then acc else buffer ~off ~len buf :: acc
...
let fcuts ~no_empty ~sep:({ len = sep_len; _ } as sep)
  ...
  let rec check_sep zanchor i k acc =
    if k > max_sep_zidx then
      ...
      scan new_start new_start
        (add_sub ~no_empty buffer ~off:(off + zanchor) ~len:(i - zanchor) acc)
    else if get_char cs (i + k) = get_char sep k then
      check_sep zanchor i (k + 1) acc
    else scan zanchor (i + 1) acc
  and scan zanchor i acc =
    if i > max_s_zidx then
      if zanchor = 0 then if no_empty && len = 0 then [] else [ cs ]
      else
        List.rev
          (add_sub ~no_empty buffer ~off:(off + zanchor) ~len:(len - zanchor)
             acc)
    else if get_char cs i = get_char sep 0 then check_sep zanchor i 1 acc
    else scan zanchor (i + 1) acc
  in
  scan 0 0 []
...
let rcuts ~no_empty ~sep:({ len = sep_len; _ } as sep)
  ...
  let rec check_sep zanchor i k acc =
    if k > max_sep_zidx then
      let off = i + sep_len in
      rscan i (i - sep_len)
        (add_sub ~no_empty buffer ~off ~len:(zanchor - off) acc)
    else if get_char cs (i + k) = get_char cs k then
      check_sep zanchor i (k + 1) acc
...
let cuts ?(rev = false) ?(empty = true) ~sep cs =
  match rev with
...
let fields ?(empty = false) ?(is_sep = is_white) ({ buffer; off; len } as cs) =
  ...
  let rec loop i end_pos acc =
    if i < 0 then
      if end_pos = len then if no_empty && len = 0 then [] else [ cs ]
      else add_sub ~no_empty buffer ~off ~len:(end_pos - (i + 1)) acc
    else if not (is_sep (get_char cs i)) then loop (i - 1) end_pos acc
    else
      loop (i - 1) i
        (add_sub ~no_empty buffer
           ~off:(off + i + 1)
           ~len:(end_pos - (i + 1))
           acc)
  in
...
let ffind sat ({ buffer = v; len; _ } as cs) =
  ...
  let rec loop i =
    ...
    else if sat (get_char cs i) then Some (buffer ~off:i ~len:1 v)
    else loop (i + 1)
  in
  loop 0
...
let find ?(rev = false) sat cs =
  match rev with true -> rfind sat cs | false -> ffind sat cs

let ffind_sub ~sub:({ len = sub_len; _ } as sub)
  ...
  if sub_len > len then None
    ...
    let rec loop i k =
      if i > max_zidx_s then None
        ...
        if get_char sub k = get_char cs (i + k) then loop i (k + 1)
        else loop (i + 1) 0
      else if get_char sub 0 = get_char cs i then loop i 1
      else loop (i + 1) 0
    in
    loop 0 0
...
let find_sub ?(rev = false) ~sub cs =
  match rev with true -> rfind_sub ~sub cs | false -> ffind_sub ~sub cs
...
let filter sat ({ len; _ } as cs) =
  if len = 0 then empty
    ...
    let max_zidx = len - 1 in
    let rec loop b k i =
      if i > max_zidx then if k = len then b else sub b 0 k
      else
